...
import fs from 'fs';
import { normalizeLeaveType } from './utils';
...
export async function sendLeaveRequestToManager({
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Nouvelle demande de congé en attente pour ${employeeName} (${normLeaveType}, ${dateRange}, ${durationStr}). Connectez-vous sur l'ERP pour la valider.`
  });
}
...
export async function sendLeaveRequestConfirmationToEmployee({
  ...
  const subject = `[ERP Step Up] Confirmation de votre demande d'absence (${normLeaveType})`;
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Votre demande de congé (${normLeaveType}, ${dateRange}, ${durationStr}) a bien été transmise à votre manager et est en attente de validation.`
  });
}
...
export async function sendLeaveDecisionToEmployee({
  ...
  const isPermission = normLeaveType.toLowerCase().includes('perm');
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Votre demande de congé (${normLeaveType}, ${dateRange}) a été ${status}. Commentaire: ${hrComment || 'Aucun'}`
  });
}
