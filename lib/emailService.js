...
export async function sendLeaveRequestToManager({
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Nouvelle demande de ${requestLabel} en attente pour ${empFirstName} (${dateRange}, ${durationStr}). Connectez-vous sur l'ERP pour la valider.`
  });
}
...
export async function sendLeaveRequestConfirmationToEmployee({
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Bonjour ${empFirstName}, votre demande de ${requestLabel} (${dateRange}, ${durationStr}) a bien été transmise à votre manager et est en attente de validation.`
  });
}
...
export async function sendLeaveDecisionToEmployee({
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Bonjour ${empFirstName}, votre demande de ${requestLabel} (${dateRange}) a été ${status}. Commentaire: ${hrComment || 'Aucun'}`
  });
}
      ...
      <table class="table-recap" style="width: 100%; border-collapse: collapse;">
        <thead>
          <tr style="background-color: #f1f5f9;">
            <th style="padding: 10px 8px; text-align: left; font-size: 12px; color: #334155; border-bottom: 2px solid #cbd5e1;">Collaborateur</th>
