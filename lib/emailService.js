...
export async function sendLeaveRequestToManager({
  ...
  const subject = `[ERP Step Up] 🔔 Nouvelle demande de ${requestLabel} : ${employeeName} (${normLeaveType})`;
  ...
  const contentHtml = `
    ...
    <div class="card">
      <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
          <td class="label" style="padding: 6px 0;">Type de demande :</td>
          <td class="value" style="padding: 6px 0;"><span style="color: #E06900; font-weight: 600;">${normLeaveType}</span></td>
        </tr>
        <tr>
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Nouvelle demande de ${requestLabel} en attente pour ${employeeName} (${normLeaveType}, ${dateRange}, ${durationStr}). Connectez-vous sur l'ERP pour la valider.`
  });
}
...
export async function sendLeaveRequestConfirmationToEmployee({
  ...
  const subject = `[ERP Step Up] Confirmation de votre demande de ${requestLabel} (${normLeaveType})`;
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Votre demande de ${requestLabel} (${normLeaveType}, ${dateRange}, ${durationStr}) a bien été transmise à votre manager et est en attente de validation.`
  });
}
...
export async function sendLeaveDecisionToEmployee({
  ...
  const contentHtml = `
    ...
    <div class="card">
      <table style="width: 100%; border-collapse: collapse;">
        <tr>
          <td class="label" style="padding: 6px 0;">Type de demande :</td>
          <td class="value" style="padding: 6px 0;">${normLeaveType}</td>
        </tr>
        <tr>
          <td class="label" style="padding: 6px 0;">Période :</td>
          <td class="value" style="padding: 6px 0;">${dateRange}</td>
  ...
  return await sendEmail({
    ...
    subject,
    html,
    text: `Votre demande de ${requestLabel} (${normLeaveType}, ${dateRange}) a été ${status}. Commentaire: ${hrComment || 'Aucun'}`
  });
}
...
export async function sendPendingLeavesDigestEmail({
  ...
  const subject = `[ERP Step Up] ${count} demande${count > 1 ? 's' : ''} d'absence (congés/permissions) en attente de validation`;
  ...
  const rowsHtml = pendingRequests.map((req, idx) => {
    ...
    const normType = normalizeLeaveType(req.leave_type);
    ...
    return `
      <tr style="background-color: ${idx % 2 === 0 ? '#ffffff' : '#f8fafc'};">
        ...
        <td style="padding: 10px 8px; border-bottom: 1px solid #e2e8f0; color: #E06900; font-weight: 600;">${normType}</td>
