...
export async function syncTimeLog(employeeId, date) {
  try {
    ...
    await runWithMutex(async () => {
      ...
      // Compute total hours worked (deducting break duration)
      let totalHoursStr = '';
      if (log.clock_in && log.clock_out) {
        ...
        if (log.break_duration) {
          const match = log.break_duration.match(/(\d+)h\s*(\d+)m/);
          if (match) {
            const breakMins = parseInt(match[1], 10) * 60 + parseInt(match[2], 10);
      ...
      const rowData = {
        ...
        [TimeLogsColumns.break_duration]: log.break_duration || '0h 00m',
