...
export async function syncEmployeeBalance(employeeId) {
  try {
    ...
    await runWithMutex(async () => {
      ...
      // Format last_monthly_credit (e.g. "2026-08" -> "08/2026")
      let formattedMonthlyCredit = '';
      if (member.last_monthly_credit) {
        const parts = member.last_monthly_credit.toString().trim().split('-');
        if (parts.length === 2) {
          formattedMonthlyCredit = `${parts[1]}/${parts[0]}`;
        } else {
          formattedMonthlyCredit = formatDateToFrench(member.last_monthly_credit);
        }
      }
      ...
      const rowData = {
        ...
        [LeaveBalancesColumns.last_monthly_credit]: formattedMonthlyCredit,
      };
  ...
  try {
    ...
    await runWithMutex(async () => {
      ...
      const rowData = {
        ...
        [LeaveRequestsColumns.created_at]: formatDateToFrench(request.created_at),
        [LeaveRequestsColumns.updated_at]: formatDateToFrench(request.updated_at),
...
export async function syncTimeLog(employeeId, date) {
  try {
    ...
    await runWithMutex(async () => {
      ...
      // Compute decimal hours in French format for easy calculation (=SOMME, etc.)
      const breakDurationDecimalStr = formatBreakDurationToDecimalHours(log.break_duration);
      const totalHoursDecimalStr = calculateTotalHoursWorkedDecimal(log.clock_in, log.clock_out, log.break_duration);
      ...
      const rowData = {
        ...
        [TimeLogsColumns.break_duration]: breakDurationDecimalStr,
        [TimeLogsColumns.total_hours]: totalHoursDecimalStr,
        [TimeLogsColumns.status]: log.status || 'Présent',
        [TimeLogsColumns.created_at]: formatDateToFrench(log.created_at)
      };
