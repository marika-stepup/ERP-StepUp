...
export async function getSheet(title) {
  ...
  if (!sheet) {
    if (title === SheetTabs.timeLogs) {
      // Auto-create missing Pointages sheet
      sheet = await withRetry(() => doc.addWorksheet({
        title,
        headerValues: Object.values(TimeLogsColumns)
      }));
      console.log(`[Google Sheets] Automatically created missing worksheet: "${title}"`);
    } else {
