...
export async function getSheet(title) {
  ...
  if (title === 'Soldes_Conges') {
    try {
      ...
      if (!updatedHeaders.includes(LeaveBalancesColumns.employee_first_name)) {
        updatedHeaders.push(LeaveBalancesColumns.employee_first_name);
        needsUpdate = true;
      }
...
export async function autoCreditContractAnniversaries() {
  ...
  for (const row of rows) {
    ...
    const hireDateUTC = new Date(parsedHireDate); // UTC midnight
    if (isNaN(hireDateUTC.getTime())) continue;

    // Use current local date normalized to UTC midnight
    ...
    if (toCredit.length > 0) {
      ...
      const newRemaining = currentRemaining + creditAmount;
...
/**
 * Automatically credits +2.5j CP to all active employees at the end of each month
 */
export async function autoCreditMonthlyLeaves() {
  ...
  for (const row of rows) {
    ...
    let startMonthStr = lastMonthlyCredit;

    if (!startMonthStr) {
      if (hireDateStr) {
        const parsedHire = parseDateFromFrench(hireDateStr); // "YYYY-MM-DD"
        if (parsedHire) {
          startMonthStr = parsedHire.substring(0, 7); // "YYYY-MM"
        }
      }
      
      if (!startMonthStr) {
        // No hire date and no last credit: initialize to current month without crediting
    ...
    if (lastMonthlyCredit) {
      const parts = lastMonthlyCredit.split('-');
      year = parseInt(parts[0], 10);
      month = parseInt(parts[1], 10); // this represents month index + 1 (the next month)
    } else {
      const parts = startMonthStr.split('-');
      year = parseInt(parts[0], 10);
      month = parseInt(parts[1], 10) - 1; // 0-indexed month of hire date
    }

    const monthsToCredit = [];
    while (true) {
      const firstDayOfMonthUTC = new Date(Date.UTC(year, month, 1));
      if (firstDayOfMonthUTC <= todayUTC) {
        const monthStr = `${year}-${String(month + 1).padStart(2, '0')}`;
        monthsToCredit.push(monthStr);
        month++;
        if (month > 11) {
          month = 0;
          year++;
    ...
    if (monthsToCredit.length > 0) {
      const currentInitial = parseSheetFloat(row.get(LeaveBalancesColumns.initial_balance));
      const currentRemaining = parseSheetFloat(row.get(LeaveBalancesColumns.remaining_balance));
      
      const creditAmount = monthsToCredit.length * 2.5;
      const newInitial = currentInitial + creditAmount;
      const newRemaining = currentRemaining + creditAmount;

      row.set(LeaveBalancesColumns.initial_balance, formatSheetFloat(newInitial));
      row.set(LeaveBalancesColumns.remaining_balance, formatSheetFloat(newRemaining));
      row.set(LeaveBalancesColumns.last_monthly_credit, monthsToCredit[monthsToCredit.length - 1]);

      console.log(`[Auto Monthly Credit] Credited +${creditAmount}j CP to ${name} (${email}) for month(s): ${monthsToCredit.join(', ')}`);
      await withRetry(() => row.save());
      await new Promise((resolve) => setTimeout(resolve, 150));
    }
  }
